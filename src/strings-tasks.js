...
function countVowels(str) {
  ...
  return Array.from(str).filter((char) => isVowel(char)).length;
}
...
function isPalindrome(str) {
  const re = /[ !?,]/g;
  const normalizeStr = str.replace(re, '').toLowerCase();
...
function invertCase(str) {
  ...
  return Array.from(str)
    .map((char) =>
      isLowerCase(char) ? char.toUpperCase() : char.toLocaleLowerCase()
    )
    .join('');
}
...
function encodeToRot13(str) {
  function convert(c) {
    ...
    const i = alphabet1.indexOf(c);
    return i === -1 ? c : alphabet2.charAt(i);
  }
  return Array.from(str)
    .map((c) => convert(c))
    .join('');
...
function getCardId(value) {
  const nValue = value.replace('0', '');
  const faces = 'A234567891JQK';
  const mast = '♣♦♥♠';

  return (
    mast.indexOf(nValue.charAt(1)) * faces.length +
    faces.indexOf(nValue.charAt(0))
  );
}
